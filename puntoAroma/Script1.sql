CREATE TABLE banner (
  ...
  SELECT p.*, c.nombre AS categoria, COUNT(DISTINCT v.aroma) AS cantidad_fragancias, MIN(CASE WHEN vtp.id_tipo_precio = 1 THEN vtp.precio END) AS precio_minorista, MIN(CASE WHEN vtp.id_tipo_precio = 2 THEN vtp.precio END) AS precio_mayorista , m.nombre AS marca
  FROM productos p 
  ...
  WHERE m.nombre = 'saphirus'
  GROUP BY p.id_producto, p.nombre, p.descripcion, c.nombre;


  SELECT p.id_producto, p.nombre , c.nombre AS categoria, COUNT(DISTINCT v.aroma) AS cantidad_fragancias, MIN(CASE WHEN vtp.id_tipo_precio = 1 THEN vtp.precio END) AS precio_minorista, MIN(CASE WHEN vtp.id_tipo_precio = 2 THEN vtp.precio END) AS precio_mayorista , m.nombre AS marca
  FROM productos p 
  JOIN categorias c ON p.id_categoria = c.id_categoria 
  JOIN marcas m ON p.id_marca = m.id_marca
  JOIN variantes v ON p.id_producto = v.id_producto 
